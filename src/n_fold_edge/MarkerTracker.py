...
class MarkerTracker:
    ...
    def __init__(self, order: int, kernel_size: int, scale_factor: float) -> None:
        ...
        self.last_marker_location: Sequence[int]
    ...
    def refine_marker_location(self) -> tuple[float, float]:
        try:
            delta = 1
    ...
    def determine_marker_orientation(self, frame: np.ndarray) -> None:
    ...
    def determine_marker_quality(self, frame: np.ndarray) -> None:
        ...
        try:
            ...
            normalised_mean_difference = np.squeeze(mean_difference / (0.5 * bright_std + 0.5 * dark_std))
    ...
    def extract_window_around_maker_location(self, frame: np.ndarray) -> np.ndarray:
        xm, ym = self.last_marker_location
        frame_tmp = np.array(frame[ym - self.y1 : ym + self.y2, xm - self.x1 : xm + self.x2])
        frame_img = frame_tmp.astype(np.uint8)
        return frame_img

    def generate_template_for_quality_estimator(self) -> tuple[np.ndarray, np.ndarray]:
