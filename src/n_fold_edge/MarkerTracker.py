...
class MarkerTracker:
    ...
    def __init__(self, order, kernel_size, scale_factor):
        ...
        (kernel_remove_arm_real, kernel_remove_arm_imag) = (
            self.generate_symmetry_detector_kernel(1, self.kernel_size)
        )
        self.kernelComplex = np.array(kernel_real + 1j * kernel_imag, dtype=complex)
        self.KernelRemoveArmComplex = np.array(
            kernel_remove_arm_real + 1j * kernel_remove_arm_imag, dtype=complex
        )
    ...
    def refine_marker_location(self):
        try:
            ...
            x = max_loc[1]
            y = max_loc[0]
            frame_sum_squared_cutout = self.frame_sum_squared[
                x - delta : x + delta + 1, y - delta : y + delta + 1
            ]
    ...
    def locate_marker(self, frame):
        ...
        self.pose = MarkerPose(
            max_loc[0], max_loc[1], self.orientation, self.quality, self.order
        )
        return self.pose
    ...
    def determine_marker_orientation(self, frame):
        ...
        self.orientation = (
            math.atan2(-real_value, imag_value) - math.pi / 2
        ) / self.order
    ...
    def determine_marker_quality(self, frame):
        ...
        try:
            ...
            temp_value_for_quality = 1 - 1 / (
                1 + math.exp(0.75 * (-7 + normalised_mean_difference))
            )
    ...
    def extract_window_around_maker_location(self, frame):
        (xm, ym) = self.last_marker_location
        frame_tmp = np.array(
            frame[ym - self.y1 : ym + self.y2, xm - self.x1 : xm + self.x2]
        )
